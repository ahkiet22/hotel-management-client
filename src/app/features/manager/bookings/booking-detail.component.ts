      ...
      <div *ngIf="!isLoading() && booking()" class="space-y-6">
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
          <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            <div class="space-y-3">
              <span class="inline-flex rounded-full px-3 py-1 text-xs font-black uppercase tracking-widest" [ngClass]="getStatusClass(booking()?.status)">
                {{ booking()?.status }}
              </span>
              ...
              <p class="text-xs font-mono text-slate-400">ID: {{ booking()?.id }}</p>
            </div>
        ...
        <div *ngIf="booking()?.status === 'Pending'" class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 space-y-6">
          <div class="flex items-start gap-4">
            ...
            <div>
              <h2 class="text-xl font-black text-slate-900">Payment QR</h2>
              <p class="mt-2 text-slate-500 font-medium">
                This booking is still pending. Staff can share this QR with the guest to complete payment.
              </p>
            </div>
          </div>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
          <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 space-y-5">
            ...
            <div *ngIf="booking()?.actualCheckIn" class="flex items-start gap-4">
              <div class="w-11 h-11 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center">
                <lucide-icon [img]="icons.CheckCircle2" class="w-5 h-5"></lucide-icon>
              </div>
              <div>
                <p class="text-xs font-black uppercase tracking-widest text-slate-400">Actual Check-in</p>
                <p class="mt-1 font-bold text-slate-900">{{ booking()?.actualCheckIn | date:'medium' }}</p>
              </div>
            </div>

            <div *ngIf="booking()?.actualCheckOut" class="flex items-start gap-4">
              <div class="w-11 h-11 rounded-2xl bg-slate-100 text-slate-600 flex items-center justify-center">
                <lucide-icon [img]="icons.CheckCircle2" class="w-5 h-5"></lucide-icon>
              </div>
              <div>
                <p class="text-xs font-black uppercase tracking-widest text-slate-400">Actual Check-out</p>
                <p class="mt-1 font-bold text-slate-900">{{ booking()?.actualCheckOut | date:'medium' }}</p>
              </div>
            </div>
          </div>

          <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 space-y-5">
            <h2 class="text-xl font-black text-slate-900">Customer & Totals</h2>
            ...
            <div class="flex items-start gap-4">
              ...
              <div>
                ...
                <p class="mt-1 font-bold text-slate-900">{{ booking()?.customerName || 'Unknown customer' }}</p>
                <p class="text-sm text-slate-500">{{ booking()?.customerEmail || '' }}</p>
                <p class="text-sm text-slate-500">{{ booking()?.customerPhone || '' }}</p>
              </div>
            </div>

            <div *ngIf="booking()?.staffName" class="flex items-start gap-4">
              <div class="w-11 h-11 rounded-2xl bg-violet-50 text-violet-600 flex items-center justify-center">
                <lucide-icon [img]="icons.User" class="w-5 h-5"></lucide-icon>
              </div>
              <div>
                <p class="text-xs font-black uppercase tracking-widest text-slate-400">Handled By</p>
                <p class="mt-1 font-bold text-slate-900">{{ booking()?.staffName }}</p>
              </div>
            </div>
            ...
            <article *ngFor="let room of booking()?.rooms" class="rounded-3xl border border-slate-100 overflow-hidden bg-slate-50">
              ...
              <div class="p-6 space-y-3">
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm leading-6 text-slate-500">{{ room.roomTypeDescription || 'Comfortable room prepared for this stay.' }}</p>
                </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 space-y-4" *ngIf="booking()?.services?.length">
          <h2 class="text-xl font-black text-slate-900">Applied Services</h2>
          <div class="space-y-3">
            <div *ngFor="let service of booking()?.services" class="flex items-center justify-between gap-4 rounded-2xl bg-slate-50 px-4 py-3">
              <div class="min-w-0">
                <p class="font-bold text-slate-900 truncate">{{ service?.serviceName || service?.name || 'Service' }}</p>
                <p class="text-sm text-slate-500">Qty {{ service?.quantity || 0 }}</p>
              </div>
              <p class="font-black text-slate-900 whitespace-nowrap">{{ (service?.price || 0) | currency:'VND' }}</p>
            </div>
          </div>
        </div>
      </div>
...
export class ManagerBookingDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.bookingService.getById(id).subscribe({
      next: (booking: any) => this.loadBookingDetail(booking),
  ...
  private loadBookingDetail(rawBooking: any) {
    ...
    forkJoin(roomTypeIds.map((id) => this.roomTypeService.getById(id))).subscribe({
      next: (roomTypes: any[]) => {
