...
export class RoomService extends BaseService {
  ...
  override getAll(query?: QueryRoomDto): Observable<PaginatedResponse<Room>> {
    ...
    return this.http
      .get<ApiResponse<PaginatedResponse<Room>>>(`${this.baseUrl}api/v1/rooms`, { params })
      .pipe(
        map((res) => res.data),
  ...
  getAllPublic(query?: QueryRoomDto): Observable<PaginatedResponse<Room>> {
    ...
    return this.http
      .get<
        ApiResponse<PaginatedResponse<Room>>
      >(`${this.baseUrl}api/v1/rooms/public`, { params, context: new HttpContext().set(IS_PUBLIC_ENABLED, true) })
      .pipe(
        map((res) => res.data),
        catchError(this.handleError),
      );
  ...
  override getById(id: string): Observable<Room> {
    return this.http.get<ApiResponse<Room>>(`${this.baseUrl}api/v1/rooms/${id}`).pipe(
      map((res) => res.data),
      catchError(this.handleError),
    );
  }
  ...
  override create(data: CreateRoomDto): Observable<any> {
    return this.http.post<ApiResponse<any>>(`${this.baseUrl}api/v1/rooms`, data).pipe(
      map((res) => res.data),
      catchError(this.handleError),
    );
  }
  ...
  override update(id: string, data: UpdateRoomDto): Observable<any> {
    return this.http.patch<ApiResponse<any>>(`${this.baseUrl}api/v1/rooms/${id}`, data).pipe(
      map((res) => res.data),
      catchError(this.handleError),
    );
  }
  ...
  override delete(id: string): Observable<any> {
    return this.http.delete<ApiResponse<any>>(`${this.baseUrl}api/v1/rooms/${id}`).pipe(
      map((res) => res.data),
      catchError(this.handleError),
    );
  }
